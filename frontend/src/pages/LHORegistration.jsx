  ...
  <script>
    ...
    async function handleSubmitApplication() {
      ...
      try {
         ...
         */
        // This page is rendered inside a srcDoc iframe. Posting the registration
        // payload to the React parent avoids browser CORS problems caused by the
        // iframe's opaque/null origin.
        window.parent.postMessage({
          type: 'LHO_SUBMIT_REGISTRATION',
          payload: {
        ...
        // The React parent performs the API request and sends the result back.
        // The success/error UI is completed by the LHO_SUBMIT_RESULT message.
...
export default function LHORegistration() {
  ...
  useEffect(() => {
    const handleMessage = async (event) => {
      if (event.source !== window && event.data?.type === "LHO_SUBMIT_REGISTRATION") {
        try {
          const response = await fetch(apiUrl("/api/lho/register"), {
            method: "POST",
          ...
          iframeRef.current?.contentWindow?.postMessage(
            {
              type: "LHO_SUBMIT_RESULT",
              success: Boolean(response.ok && data.success),
              data,
              error: !response.ok || !data.success
                ? (data.message || `Unable to register LHO (HTTP ${response.status}).`)
                : ""
            },
            "*"
          );
          ...
          iframeRef.current?.contentWindow?.postMessage(
            {
              ...
              success: false,
              data: {},
              error: error?.message || "Cannot connect to the EMMC backend."
            },
            "*"
          );
        }
      ...
      if (event.data?.type === "LHO_SUBMIT_RESULT") {
        const result = event.data;
        if (!result.success) {
          const box = iframeRef.current?.contentDocument?.getElementById("formAlertBox");
          if (box) {
            box.textContent = result.error || "LHO registration failed.";
            box.className = "mb-6 p-3 rounded-md border text-xs bg-red-50 text-red-800 border-red-200";
            box.classList.remove("hidden");
          }
          const button = iframeRef.current?.contentDocument?.querySelector('button[onclick="handleSubmitApplication()"]');
          if (button) {
        ...
        const appId = result.data?.lhoId;
        if (!appId) {
          const box = iframeRef.current?.contentDocument?.getElementById("formAlertBox");
          if (box) {
            box.textContent = "Registration succeeded, but the backend did not return an LHO ID.";
            box.className = "mb-6 p-3 rounded-md border text-xs bg-red-50 text-red-800 border-red-200";
            box.classList.remove("hidden");
          }
          return;
        ...
        if (doc) {
          const appIdEl = doc.getElementById("finalAppId");
          if (appIdEl) appIdEl.textContent = appId;
          const timeEl = doc.getElementById("submissionTime");
          if (timeEl) {
            const now = new Date();
            timeEl.textContent = now.toLocaleDateString() + " " + now.toLocaleTimeString();
          }
          doc.getElementById("stepSection5")?.classList.add("hidden");
          doc.getElementById("successScreen")?.classList.remove("hidden");
          const progressLine = doc.getElementById("progressLine");
