...
export default function TrafficPoliceTracker({ onStartMap, onPoliceLocationChange, mapOpen = false, logoutNonce = 0 }) {
  ...
  const enableMobileAlerts = async () => {
    ...
    try {
      ...
      let healthResponse;
      try {
        healthResponse = await fetch(`${BACKEND_URL}/api/health`, { cache: "no-store" });
      } catch (networkError) {
        throw new Error(`Render Push Backend unreachable/CORS blocked: ${networkError?.message || "Network error"}`);
      }
      ...
      let keyResponse;
      try {
        keyResponse = await fetch(`${BACKEND_URL}/api/push/public-key`, { cache: "no-store" });
      } catch (networkError) {
        throw new Error(`VAPID public-key request blocked: ${networkError?.message || "Network error"}`);
      }
      if (!keyResponse.ok) {
      ...
      // Reuse a valid existing subscription. If the browser has a stale
      // subscription (common after redeploying the site/service-worker),
      // remove it and create a fresh one with the current Render VAPID key.
      let subscription = await registration.pushManager.getSubscription();
      const applicationServerKey = urlBase64ToUint8Array(keyData.publicKey.trim());
